...
void MemManage_Handle(void)         __attribute__((weak, alias("Default_Handler")));
...
void EXTI23_IRQHandler(void)       __attribute__((weak, alias("Default_Handler")));
...
void Reset_Handler(void)
{
	...
	uint32_t *pSrc = (uint32_t*)&_edata; //FLASH
	...
	//Init the std library like printf scanf
	__libc_init_array();
	//call main()
	main();
}
